...
analyze_clpn_network <- function(
  ...
  variable_labels <- paste0(lognames, " (", node_names, ")")
  ...
  bridge_ei_1step <- bridge_results[["Bridge Expected Influence (1-step)"]]
  bridge_ei_2step <- bridge_results[["Bridge Expected Influence (2-step)"]]
  
  names(bridge_ei_1step) <- t1_vars_base
  names(bridge_ei_2step) <- t1_vars_base
  ...
  all_centrality <- data.frame(
    ...
    BridgeEI_1step       = bridge_ei_1step,
    BridgeEI_2step       = bridge_ei_2step
  )
  ...
  bridge_order <- order(all_centrality$BridgeEI_1step, decreasing = TRUE)
  ...
  centrality_z_save <- all_centrality_z %>%
    select(Variable, OutExpectedInfluence, InExpectedInfluence, BridgeEI_1step, BridgeEI_2step)
  
  write.csv(centrality_z_save,
            file.path(output_dir, "centrality_OEI_IEI_BEI_zscore.csv"),
            row.names = FALSE)
  ...
  all_centrality_long <- all_centrality_z %>%
    ...
    mutate(
      Measure = factor(Measure, 
                       levels = c("OutExpectedInfluence", "InExpectedInfluence", "BridgeEI_1step", "BridgeEI_2step"),
                       labels = c("Out-Expected Influence", "In-Expected Influence", "Bridge Expected Influence (1-step)",
  ...
  centrality_long_oei_iei <- all_centrality_z %>%
    select(
      Variable,
      VariableLabel,
      Position,
  ...
  plot_oei_iei <- ggplot(
    centrality_long_oei_iei,
    aes(x = ZScore, y = reorder(VariableLabel, -Position))
  ) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "gray70") +
    geom_point(aes(shape = Measure), size = 2.5, color = "black") +
    geom_path(aes(group = Measure), linewidth = 0.8) +
    ...
    scale_shape_manual(
      values = c(
        "Out-Expected Influence" = 16,
        "In-Expected Influence"  = 17
      )
    )
  pdf_file_oei_iei <- file.path(output_dir, "plot_centrality_OEI_IEI.pdf")
  pdf(pdf_file_oei_iei, width = 10, height = 8)
  print(plot_oei_iei)
  dev.off()
  
  cat("OEI & IEI centrality plot saved to:", pdf_file_oei_iei, "\n")
  centrality_long_bei <- all_centrality_z %>%
    select(
      Variable,
      VariableLabel,
      Position,
    ...
    pivot_longer(
      cols = c(BridgeEI_1step, BridgeEI_2step),
      names_to = "Measure",
      values_to = "ZScore"
    ) %>%
    mutate(
      Measure = factor(
        Measure,
        levels = c("BridgeEI_1step", "BridgeEI_2step"),
        labels = c(
  ...
  plot_bei <- ggplot(
    ...
    facet_grid(. ~ Measure, scales = "free_x") +
    labs(
      x = "Bridge Centrality (z-scored)",
      y = NULL
    ) +
    my_theme +
    scale_shape_manual(
      values = c(
        "Bridge Expected Influence (1-step)" = 15,
        "Bridge Expected Influence (2-step)" = 18
      )
    )
  pdf_file_bei <- file.path(output_dir, "plot_centrality_BEI_1step_2step.pdf")
  pdf(pdf_file_bei, width = 10, height = 8)
  print(plot_bei)
  dev.off()
  
  cat("BEI (1-step & 2-step) centrality plot saved to:", pdf_file_bei, "\n")
  ...
  pdf(pdf_file, width = 18, height = 8)
