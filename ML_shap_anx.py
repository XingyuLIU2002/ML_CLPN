...
def save_plot(filename, fig=None, dpi=600, flag_show=True):
    ...
    os.makedirs("ml_anx/plots_anx_cat", exist_ok=True)
    
    full_filename = f"ml_anx/plots_anx_cat/{filename}.png"
...
def save_shap_results_to_excel(importance_df, interpretation_df, shap_values_df=None, 
    ...
    os.makedirs("ml_anx/output", exist_ok=True)
    
    filename = f"ml_anx/output/{filename_prefix}_{timestamp}.xlsx"
